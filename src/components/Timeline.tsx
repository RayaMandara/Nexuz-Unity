...
const Timeline = () => {
  ...
  // Fungsi untuk mendapatkan terjemahan berdasarkan judul asli
  const getTranslatedTimeline = (event: TimelineEvent) => {
    const timelineData = t("timeline_data");
    
    // Cari terjemahan berdasarkan judul asli (Indonesia)
    if (timelineData && typeof timelineData === 'object') {
      const translated = timelineData[event.title];
      if (translated) {
        return {
    ...
    // Jika tidak ada terjemahan, gunakan data asli
    return {
      title: event.title,
    ...
    return (
      <section id="timeline" className="py-24 px-6 bg-black">
        <div className="container mx-auto max-w-5xl text-center">
          <div className="text-gray-400">{t("timeline_loading")}</div>
        </div>
      </section>
    );
  }
        ...
        {timelineEvents.length === 0 ? (
          ...
          <div ref={ref} className="relative">
            ...
            {timelineEvents.map((event, index) => {
              const translated = getTranslatedTimeline(event);
                ...
                >
                  ...
                  <div className={`w-full md:w-1/2 ${index % 2 === 0 ? "md:pr-12" : "md:pl-12"}`}>
                    <div className="bg-gradient-to-br from-white/5 to-white/10 rounded-2xl p-6 border border-white/10 hover:border-white/30 transition-all duration-300 backdrop-blur-sm">
                      ...
                      <p className="text-gray-400 text-sm mb-3 notranslate">{event.year}</p>
